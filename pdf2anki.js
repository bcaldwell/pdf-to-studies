  ...
  return new Promise((resolve, reject) => {
    pdfDocument.getPage(pageIndex).then(function (page) {
      ...
      renderTask.promise.then(function() {
        ...
        fs.writeFile(`${dir}/${stackName}-${pageIndex}_a.png`, image, (error) => {
          if (error) {
            ...
            fs.writeFile(`${dir}/${stackName}-${pageIndex}_b.png`, image, (error) => {
              if (error) {
                console.error('Error: ' + error);
                reject();
              } else {
...
let dir = "/Users/benjamincaldwell/Library/Application Support/Anki2/Benjamin/collection.media"
...
Promise.all([loadingTask.promise, dirPromise]).then(async function(pdfDocument) {
  ...
  let csvFile = fs.createWriteStream(`${stackName}.txt`);
  // csvFile.write("1 Text, 1 Image, 2 Text, 2 Image\n")
  for (i = 1; i <= pdfDocument.numPages; i++) {
    csvFile.write(`<img src="${stackName}-${i}_a.png">;<img src="${stackName}-${i}_b.png">\n`)
  }
  csvFile.end()
